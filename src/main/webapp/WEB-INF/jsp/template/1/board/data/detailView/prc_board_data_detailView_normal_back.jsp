...
<%@ include file="/WEB-INF/jsp/taglib/taglib.jspf"%>
	<div class="detailViewTilteBox">
    	<div class="topBox clearfix">
        	<p class="left_1">
            	<span>${language_default_Subject }</span><font class="contents"><c:out value="${BoardNotice.BN_TITLE}" escapeXml="true"/></font>
            </p>
            <c:if test="${currentMenu.MN_MAINKEY ne 'MN_0000000278' }"> <!-- 경영공시쪽 게시판은 작성일 X -->
            <p class="right_1">
            	<span>${language_default_date }</span>${fn:substring(BoardNotice.BN_REGDT,0,10) }
            </p>
            </c:if>
        </div>
        ...
        <div class="bottomBox clearfix">
        	<div class="row clearfix ">
            	<p class="left_1">
                	<span>${language_default_writer }</span> ${BoardNotice.BN_UI_NAME }
                </p>
                <p class="right_1">
                	<span>${language_default_hits }</span> ${BoardNotice.BN_HITS }
                </p>
            </div>
            <c:if test="${BoardType.BT_UPLOAD_YN eq 'Y' && fn:length(FileSub) gt 0 }">
            <div class="row clearfix mgT10">
            	<span>${language_default_Attachment }</span>
