			...
			<div class="innerContainer clearfix">
				...
				<form:form id="Form" name="Form" method="post">
					...
					<c:choose>
						...
						<c:when test="${BoardType.BT_CODEKEY eq BOARD_TYPE_ACTIVE }">
							<!-- 사회공헌 활동(이미지/동영상 형) -->
							<%@ include file="listType/prc_board_data_listView_active.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq 'SC_0000000413' }">
							<!-- D-day 카운트 형 -->
							<%@ include file="listType/prc_board_data_listView_list_dday.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq 'SC_0000000415' }">
							<!-- 채용부분 -->
							<%@ include file="listType/prc_board_data_listView_list_hire.jsp"%>
						</c:when>
						<c:otherwise>
...
$(function(){
	...
	$(".left_1").find('li').removeClass('active');
	
	if('${SearchKeywordss}' == '공사'){
		$(".taps2").addClass('active');
		$("#caption_board").html("<caption>조달계약실적 게시판 - 공사</caption>");
	}else if('${SearchKeywordss}' == '용역'){
		$(".taps3").addClass('active');
		$("#caption_board").html("<caption>조달계약실적 게시판 - 용역</caption>");
	}else if('${SearchKeywordss}' == '물품'){
		$(".taps4").addClass('active');
		$("#caption_board").html("<caption>조달계약실적 게시판 - 물품</caption>");
	}else{
		$(".taps1").addClass('active');
		$("#caption_board").html("<caption>조달계약실적 게시판 - 전체</caption>");
	}
	//최신순, 오래된순
...
//e-book연결
function e_book(key){
	if(key == 'BN_0000033173'){
		window.open("/e-book1.jsp");
	}else if(key == 'BN_0000033174'){
		window.open("/e-book2.jsp");
	}else if(key == 'BN_0000033176'){
