...
function hireInsert(){
	$("#Form").attr("action", "/jact/hire/intro.do");
	$("#TH_CLASSTYPE").val($("#classType").val());
...
//합격 확인 페이지 넘어가기
function hireResult(){
	$("#Form").attr("action", "/jact/hire/login2.do");
...
<div class="innerContainer clearfix">
				<form:form id="Form" name="Form" method="post">
					...
					<input type="hidden" name="TH_BNKEYNO2" id="TH_BNKEYNO2" value="${BoardNotice.BN_HIRE_KEY }">
					...
					<input type="hidden" id="TH_CLASSTYPE" name="TH_CLASSTYPE" value="" />
				...
				<div class="boardDetailViewBox">
					<c:choose>
						<c:when test="${BoardType.BT_CODEKEY eq BOARD_TYPE_GALLERY }">
							<!-- 동영상형 필요할경우 타입 분기처리 -->
							<%@ include file="detailView/prc_board_data_detailView_normal_movie.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq BOARD_TYPE_GALLERY_IMG }">
							<!-- 이미지형 필요할경우 타입 분기처리 -->
							<%@ include file="detailView/prc_board_data_detailView_normal_picture.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq BOARD_TYPE_SIGN }">
							<!-- 조달계약실적 -->
							<%@ include file="detailView/prc_board_data_detailView_sign.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq 'SC_0000000413' }">
							<!-- Dday -->
							<%@ include file="detailView/prc_board_data_detailView_dday.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq 'SC_0000000415' }">
							<!-- Dday -->
							<%@ include file="detailView/prc_board_data_detailView_hire.jsp"%>
						</c:when>
						<c:otherwise>
...
<!-- 게시판 버튼 -->
<div class="inner1200" style="margin-bottom: 70px;">
	<div class="board-detial-view">
		<div class="btn-b">
			<c:if test="${not empty nextBoardNotice }">
				<fmt:parseNumber
					value="${fn:substring(nextBoardNotice.BN_KEYNO,5,20)}"
					var="numberType" />
				<button type="button" class="btn-1 prev"
					onclick="location.href='${tilesUrl}/Board/${numberType }/detailView.do'">
					<span class="icon"><i class="xi-angle-left"></i></span> 이전글
				</button>
			</c:if>
			
			<c:if test="${currentMenu.MN_KEYNO eq 'MN_0000001510'}">
				<c:if test="${not empty Ctype }">
					<c:if test="${TommorrowDay >= 0  && Preday <= 0}">	
						<c:if test="${empty BoardNotice.BN_HIRE_KEY}">
							<button type="button" onclick="hireInsert();" class="btn_support purple" title="지원서 작성"><i class="xi-pen"></i> 지원서 작성</button>
							<button type="button" class="btn_support gray" onclick="hireUpdate();" title="지원서 수정"><i class="xi-external-link"></i> 지원서 수정</button>	
						</c:if>
						<c:if test="${not empty BoardNotice.BN_HIRE_KEY }">
							<button type="button" type="button" class="btn_support purple" onclick="hireResult();" title="합격 확인"><i class="xi-external-link"></i> 합격 확인</button>
						</c:if>
					</c:if>
				</c:if>
				<button type="button" class="btn_support gray" title="목록" onclick="pf_back()"><i class="xi-list-square"></i> 목록</button>
			</c:if>
			
			<c:if test="${currentMenu.MN_KEYNO ne 'MN_0000001510'}">
				<button type="button" class="btn-list" onclick="pf_back()">목록보기</button>
			</c:if>
			...
			<c:if test="${not empty prevBoardNotice }">
				<fmt:parseNumber
					value="${fn:substring(prevBoardNotice.BN_KEYNO,5,20)}"
					var="numberType" />
				<button type="button" class="btn-1 next"
					onclick="location.href='${tilesUrl}/Board/${numberType }/detailView.do'">
					다음글 <span class="icon"><i class="xi-angle-right"></i></span>
				</button>
			</c:if>

			<!-- 수정,삭제,답글 -->
			<fmt:parseNumber value="${fn:substring(BoardNotice.BN_KEYNO, 4, 20)}"
			...
			<c:if test="${BoardNotice.BN_USE_YN eq 'Y' && ( not empty BoardNotice.BN_PWD || BoardNotice.BN_REGNM == userInfo.UI_KEYNO || userInfo.isAdmin eq 'Y') }">
				<button class="${currentMenu.MN_KEYNO eq 'MN_0000001510'? 'btn_support gray':'btn-list'}" type="button"
					onclick="pf_UpdateView('${BN_KEYNO_NUMBERTYPE}')">수정</button>
				<button class="${currentMenu.MN_KEYNO eq 'MN_0000001510'? 'btn_support gray':'btn-list'}" type="button"
					onclick="pf_deleteMove('${BN_KEYNO_NUMBERTYPE}')">삭제</button>
			</c:if>
			
			<c:if
				test="${BoardType.BT_REPLY_YN eq 'Y' && BoardNotice.BN_IMPORTANT ne 'Y' }">
				<button class="btn-list" type="button" onclick="pf_replyWrite()">
					답글</button>
			</c:if>
		</div>
	</div>
