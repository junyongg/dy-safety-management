...
<%-- 									<span>${model.COLUMN_NAME }</span> --%>
									<c:choose>
										...
										<c:otherwise> 
											<fmt:parseDate var="date1" value="${model.BD_DATA }" pattern="yyyy-MM-dd"/>
											...
											<c:set var="Dday" value="${endDate - nowDate}"/>
											
											<c:if test="${Dday >= 0 }">
												*현재기준으로 종료일<b> <c:out value="${model.BD_DATA }" escapeXml="true"/></b>까지  <b>${Dday }일</b> 남았습니다.
											</c:if>
										</c:otherwise>
									</c:choose>
								</p>
