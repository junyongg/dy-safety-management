							        ...
							        <c:if test="${bcl.BL_TYPE ne BOARD_COLUMN_TYPE_TITLE}">
								        <c:forEach items="${BoardColumnDataList }" var="model" varStatus="status">
									        <c:if test="${not empty model.BD_DATA && bcl.BL_KEYNO eq model.BD_BL_KEYNO}">	
									        	<c:if test="${bcl.BL_COLUMN_NAME eq '도급업체주소' || bcl.BL_COLUMN_NAME eq '면허종류' }">
									        		<tr>
									        	...
									        	<c:if test="${(count mod 2 eq 1 )}">
			                            			...
			                            			<c:if test="${bcl.BL_COLUMN_NAME ne '도급업체주소' && bcl.BL_COLUMN_NAME ne '면허종류'}">
													<th>${model.COLUMN_NAME }</th>
													<c:choose>
														<c:when test="${model.BD_BL_TYPE eq BOARD_COLUMN_TYPE_CHECK }"><td><c:out value="${fn:replace(model.BD_DATA,'|',',' ) }"/></td></c:when>
														<c:when test="${model.BD_BL_TYPE eq BOARD_COLUMN_TYPE_CHECK_CODE }"><td><c:out value="${fn:replace(model.BD_DATA,'|',',' ) }"/></td></c:when>
														<c:when test="${model.BD_BL_TYPE eq BOARD_COLUMN_TYPE_NUMBER }">
															<c:if test="${model.BD_BL_KEYNO eq  PRICE_PERCENT2}">
																<td><c:out value="${model.BD_DATA}"/>%</td>
															</c:if>
															<c:if test="${model.BD_BL_KEYNO ne  PRICE_PERCENT2}">
