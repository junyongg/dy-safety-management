...
function pf_boardDataInsert(){
	...
	var resultCal = null;
	if(Percent != Infinity ){
		resultCal = Percent.toFixed(2);
	}else{
		resultCal = 0;		
	}
	$(".PRICE_PERCENT").val(resultCal);
...
function pf_boardDataUpdate(){
	...
	if(confirm("게시물을 수정하시겠습니까?")){
		...
		if(fCnt > 0){
			...
			var fs_fmkey = $('#BN_FM_KEYNO').val();
			$('#FM_KEYNO').val(fs_fmkey);
			pf_checkFile();
		}
				...
				<div class="subContentInner">
				    <div class="boardWriteBox"> <!-- 글 작성 시작 -->
							<table class="tbl_board_Write">
				                 ...
				                 <tbody>
				                 	<c:if test="${action eq 'insert' && Menu.MN_KEYNO eq BOARD_INQUIRE}">
					                 	<tr>
					                     	<th><label for="BN_NAME"><span><i class="fal fa-check"></i></span> 작성자</label></th>
					                         <td colspan="3">
					                         	<input type="text"  class=" txtDefault txtWUploadM" name="BN_NAME" id="BN_NAME" maxlength="10">
					                         </td>
					                    </tr>
					                 	<tr>
					                     	<th><label for="BN_PWD"><span><i class="fal fa-check"></i></span> 비밀번호입력</label></th>
					                         <td>
					                         	<input type="password" class="txtDefault txtWUploadPwd" name="BN_PWD" id="BN_PWD" maxlength="10">
					                         	<p class="passParaph">(4자리이상 입력해주세요.)</p>
					                         </td>
					                     	<th><label for="BN_PWD2"><span class="redSpan"><i class="fal fa-check"></i></span> 비밀번호확인</label></th>
					                         <td>
				                	...
				                	<c:if test="${userInfo.isAdmin eq 'Y' && Menu.MN_KEYNO ne BOARD_INQUIRE }">
										<tr>
					                     	<th><label for="BN_IMPORTANT"><span><i class="fal fa-check"></i></span> 공지사용</label></th>
					                        <td>
					                     	...
					                     	<th><label for="BN_IMPORTANT_DATE"><span><i class="fal fa-check"></i></span> 공지 종료일</label></th>
					                        <td>
									...
									<tr>
				                     	<th><label for="txtUpSubject">비밀글 여부</label></th>
				                         <td colspan="3">
					                     ...
					                     <c:forEach items="${BoardColumnList }" var="model" varStatus="status">
												...
												<c:if test="${model.BL_TYPE eq BOARD_COLUMN_TYPE_TITLE }">
													<tr>
					                     				<th><label for="BD_DATA${model.BL_KEYNO }"><span><i class="fal fa-check"></i></span> ${model.BL_COLUMN_NAME }</label></th>
											        	<td colspan="3">	
														<c:if test="${not empty BoardNotice }">
															<c:if test="${action eq 'insert' }">
												...
												<c:if test="${model.BL_TYPE eq BOARD_COLUMN_TYPE_NOTICE_NUMBER }">
													<tr>
					                     				<th><label for="BD_DATA${model.BL_KEYNO }">${model.BL_COLUMN_NAME }</label></th>
												        <td colspan="3">	
												...
												<c:if test="${model.BL_TYPE eq BOARD_COLUMN_TYPE_TEXT }">
													<tr>
					                     				<th><label for="BD_DATA${model.BL_KEYNO }">${model.BL_COLUMN_NAME }</label></th>
												        <td colspan="3">	
															<input type="text" class="${BL_VALIDATE} txtDefault txtWlong_1 txtWidth" data-title="${model.BL_COLUMN_NAME }" id="BD_DATA${model.BL_KEYNO }" name="BD_DATA" value="" maxlength="70">
														</td>
													</tr>	
												</c:if>
												...
												<c:if test="${model.BL_TYPE eq BOARD_COLUMN_TYPE_NUMBER }">
													<tr>
												        ...
												        <td colspan="3">
												        	<c:choose>
																<c:when test="${model.BL_KEYNO eq PRICE_A || model.BL_KEYNO eq PRICE_A2}">
																	<input type="number" class="${BL_VALIDATE} txtDefault txtWlong_1 BD_DATA_NUMBER PRICE_A" data-title="${model.BL_COLUMN_NAME }" id="BD_DATA${model.BL_KEYNO }" name="BD_DATA" value="" oninput="cf_maxLengthCheck(this)" max="99999999999" maxlength="11">
																</c:when>
																<c:when test="${model.BL_KEYNO eq PRICE_B || model.BL_KEYNO eq PRICE_B2}">
																	<input type="number" class="${BL_VALIDATE} txtDefault txtWlong_1 BD_DATA_NUMBER PRICE_B" data-title="${model.BL_COLUMN_NAME }" id="BD_DATA${model.BL_KEYNO }" name="BD_DATA" value="" oninput="cf_maxLengthCheck(this)" max="99999999999" maxlength="11">
																</c:when>
																<c:when test="${model.BL_KEYNO eq PRICE_PERCENT || model.BL_KEYNO eq PRICE_PERCENT2}">
																	<input type="number" class="${BL_VALIDATE} txtDefault txtWlong_1 BD_DATA_NUMBER PRICE_PERCENT" data-title="${model.BL_COLUMN_NAME }" id="BD_DATA${model.BL_KEYNO }" name="BD_DATA" value="" oninput="cf_maxLengthCheck(this)" max="99999999999" maxlength="11" readonly="readonly">
																</c:when>
																<c:otherwise>
					                    ...
					                    <c:if test="${BoardType.BT_UPLOAD_YN == 'Y' && not empty userInfo}">
					                    	<tr>
						                     	<th><label for="txtUpSubject">첨부파일</label></th>
						                         <td colspan="3">
