                ...
                <div class="detailViewWrap"> <!-- 상세보기 시작 -->
                    ...
                    <div class="detail_infoBox_contract"> <!-- 게시판 정보 -->
                            	...
                            	<c:forEach items="${BoardColumnList }" var="bcl">
							        <c:if test="${bcl.BL_TYPE ne BOARD_COLUMN_TYPE_TITLE}">
								        ...
								        <c:if test="${not empty model.BD_DATA && bcl.BL_KEYNO eq model.BD_BL_KEYNO}">
								        	<c:if test="${model.COLUMN_NAME ne '계약율(%)(B/A)'}">
							        		<tr>
							        		<c:choose>
							        			<c:when test="${model.COLUMN_NAME eq '계약일자'}">
								        			<th rowspan="4" class="writeVertical"style="border-right: 1px solid #ddd;">계<br>약<br>개<br>요</th>
								        			<c:set var="rowspan" value="1"/>
								        			<c:set var="colspan1" value="1"/>
								        			<c:set var="colspan2" value="3"/>
								        		</c:when>
								        		<c:when test="${model.COLUMN_NAME eq '계약기간'}">
								        			<c:set var="rowspan" value="1"/>
								        			<c:set var="colspan1" value="1"/>
								        			<c:set var="colspan2" value="3"/>
								        		</c:when>
								        		<c:when test="${model.COLUMN_NAME eq '예정가격(A)' || model.COLUMN_NAME eq '계약금액(B)'}">
								        			<c:set var="rowspan" value="1"/>
								        			<c:set var="colspan1" value="1"/>
								        			<c:set var="colspan2" value="1"/>
								        		</c:when>
								        		
								        		<c:when test="${model.COLUMN_NAME eq '업체명'}">
								        			<th rowspan="3" class="writeVertical" style="border-right: 1px solid #ddd;">계<br>약<br>상<br>대<br>자</th>
								        			<c:set var="rowspan" value="1"/>
								        			<c:set var="colspan1" value="1"/>
								        			<c:set var="colspan2" value="3"/>
								        		</c:when>
								        		<c:when test="${model.COLUMN_NAME eq '대표자' || model.COLUMN_NAME eq '주소'}">
								        			<c:set var="rowspan" value="1"/>
								        			<c:set var="colspan1" value="1"/>
								        			<c:set var="colspan2" value="3"/>
								        		</c:when>
								        		...
								        		<c:otherwise>
								        			<c:set var="rowspan" value="1"/>
								        			<c:set var="colspan1" value="2"/>
								        			<c:set var="colspan2" value="3"/>
								        		</c:otherwise>
							        		</c:choose>
							        		
							        		
												<th colspan="${colspan1 }" >${model.COLUMN_NAME }</th>
												<c:choose>
													...
													<c:when test="${model.BD_BL_TYPE eq BOARD_COLUMN_TYPE_NUMBER }">
														<td><fmt:formatNumber value="${model.BD_DATA}" pattern="#,###"/>원</td>
													</c:when>
													...
													<c:otherwise>
														<td colspan="${colspan2 }">
															<c:out value="${model.BD_DATA }" escapeXml="true"/>
														</td>
													</c:otherwise>
												</c:choose>
												<c:if test="${model.COLUMN_NAME eq '예정가격(A)'}">
													<c:forEach items="${BoardColumnDataList }" var="model2" varStatus="status">
													<c:if test="${model2.COLUMN_NAME eq '계약율(%)(B/A)'}">
													<th rowspan="2">계약율(%)<br>(B/A)</th>
							           				<td rowspan="2">
														<c:out value="${model2.BD_DATA } " escapeXml="true"/>%
													</td>
													</c:if>
													</c:forEach>
							           			</c:if>
                     ...
                     <div class="btnUploadBox">
							...
							<button class="btnDefault bg_gray_01 btnSizeM_3" type="button" onclick="pf_UpdateView('${BN_KEYNO_NUMBERTYPE}')">
						 		수정
							</button>
							<button class="btnDefault bg_gray_01 btnSizeM_3" type="button" onclick="pf_deleteMove('${BN_KEYNO_NUMBERTYPE}')">
								삭제
							</button>
						</c:if>
	                   	<c:if test="${userInfo.isAdmin eq 'Y'}">
	                    	<c:if test="${BoardType.BT_REPLY_YN eq 'Y' && BoardNotice.BN_IMPORTANT ne 'Y' }">
							<button class="btnDefault bg_gray_01 btnSizeM_3" type="button" onclick="pf_replyWrite()">
								답글
							</button>
							</c:if>
	               		</c:if>
                    ...
                    <c:if test="${BoardType.BT_UPLOAD_YN eq 'Y' && fn:length(FileSub) gt 0 }">
	                    <div class="detail_attachBox"> <!-- 첨부파일 내용 -->
	                    	<ul class="detailAttachUl">
	                    		<c:forEach items="${FileSub}" var="fs">
	                    			<c:choose>
	                    				<c:when test="${fs.FS_EXT eq 'hwp'}">
