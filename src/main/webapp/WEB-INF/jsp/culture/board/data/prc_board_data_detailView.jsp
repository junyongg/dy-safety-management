				...
				<div class="boardDetailViewBox">
					<c:choose>
						...
						<c:when test="${BoardType.BT_CODEKEY eq BOARD_TYPE_LETTER }">
							<!-- 학술보고서 -->
							<%@ include file="detailView/prc_board_data_detailView_report.jsp"%>
						</c:when>
						<c:when test="${BoardType.BT_CODEKEY eq 'SC_0000000412' }">
							<!-- 연구소 소식 -->
							<%@ include file="detailView/prc_board_data_detailView_link.jsp"%>
						</c:when>
						<c:otherwise>
		...
		<div class="board-detial-view">
			<div class="btn-b">
				<c:if test="${not empty nextBoardNotice }">
					...
					<button type="button" class="btn-1 prev" onclick="location.href='${tilesUrl}/Board/${numberType }/detailView.do'">
				...
				<c:if test="${not empty prevBoardNotice }">
					...
					<button type="button" class="btn-1 next" onclick="location.href='${tilesUrl}/Board/${numberType }/detailView.do'">
...
<%-- 					</c:if> --%>
					<c:if test="${BoardNotice.BN_USE_YN eq 'Y' && ( not empty BoardNotice.BN_PWD || BoardNotice.BN_REGNM == userInfo.UI_KEYNO || userInfo.isAdmin eq 'Y') }">
						<button class="btn-list" type="button" onclick="pf_UpdateView('${BN_KEYNO_NUMBERTYPE}')">
					 		수정
						</button>
						<button class="btn-list" type="button" onclick="pf_deleteMove('${BN_KEYNO_NUMBERTYPE}')">
							삭제
						</button>
					</c:if>
