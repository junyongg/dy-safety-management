								...
								<div class="board-write-box">
									...
									<div class="table-box">
										<table class="tbl_write_1">
											...
											<c:forEach items="${BoardColumnList }" var="model"
												...
												<c:if test="${model.BL_TYPE eq BOARD_COLUMN_TYPE_TEXT }">
													<tr>
														...
														<td><input type="text" class="${BL_VALIDATE} txtDefault txtWlong_1" data-title="${model.BL_COLUMN_NAME }"
															id="BD_DATA${model.BL_KEYNO }" name="BD_DATA" value="" maxlength="70" style="width:50%;"></td>
													</tr>
												</c:if>
											...
											</c:if> <c:if test="${BoardType.BT_SECRET_YN == 'Y'}">
												<tr>
...
<script>
	...
	$(function(){
		if("${userInfo.isAdmin}" == "Y"){
			/* if("${currentMenu.MN_MAINKEY}" == 'MN_0000001570' || "${currentMenu.MN_MAINKEY}" == 'MN_0000001583' || "${currentMenu.MN_MAINKEY}" == 'MN_0000001626'){ */
				$(".hidden_col").show();	
			/* }else{
				$(".hidden_col").hide();	
				$(".title_tr").css('border-top','1px solid #6093f0');
			} */
		}else{
			$(".hidden_col").hide();
			$(".title_tr").css('border-top','1px solid #6093f0');
