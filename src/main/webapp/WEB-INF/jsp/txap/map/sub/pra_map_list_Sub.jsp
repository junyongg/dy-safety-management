...
<table id="dt_basic" class="pagingTable table table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr>
			...
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="3" placeholder="분류"/>
			</th>
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="4" placeholder="제목"/>
			</th>
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="5" placeholder="프로젝트 명"/>
			</th>
			<th class="hasinput">
			</th>
		</tr>
		...
		<tr>
			...
			<th class="arrow" data-index="3">분류</th>
			<th class="arrow" data-index="4">제목</th>
			<th class="arrow" data-index="5">프로젝트 명</th>
	...
	<tbody>
		...
		<tr>
			...
			<td>${model.MMS_TYPE}</td>
			<td><a href="javascript:;" onclick="pf_actionViewPage('${model.MMS_KEYNO}')">${model.MMS_TITLE}</a></td>
			<td class="">${model.MMA_TITLE}</td>
			<td>
				<input type="checkbox" name="CheckData" value="${model.MMS_KEYNO }" >
			</td>
		</tr>
		</c:forEach>
	</tbody>
...
function pf_DataDelete(){
	...
	if(list.length > 0){
		$.ajax({
			type	:	"post",
			url		:	"/txap/map/checkDeleteSub.do?${_csrf.parameterName}=${_csrf.token}",
			traditional : true,
			data	:   {
				"keylist" : keyno
			},
