...
<%@ include file="pra_banner_listView_main.jsp" %>

<%@ include file="pra_banner_listView_sub.jsp" %>
...
function pf_bannerMain(key){
	location.href="/txap/operation/banner/insertView.do?BM_KEYNO=" + key;
}
...
function pf_useBanner(key, mainkey, type, use, level){
	...
	$("#BM_TYPE").val(type);
	$('#actionForm').attr('action','/txap/operation/banner/check.do');
	$('#actionForm').submit()
}

function pf_DeleteBanner(key){
	$('#BM_KEYNO').val(key);
	$('#actionForm').attr('action','/txap/operation/banner/deleteAjax.do');
	$('#actionForm').submit()
}

function pf_bannerRegist(){
	$("#actionForm").attr("action", "/txap/operation/banner/insertView.do");
	$("#actionForm").submit();
}
...
function fn_updateLevel(keyno,level,afterLevel, mainkey, type){
	...
	$("#actionForm").attr("action", "/txap/operation/banner/level/update.do");
	$("#actionForm").submit();
}	
