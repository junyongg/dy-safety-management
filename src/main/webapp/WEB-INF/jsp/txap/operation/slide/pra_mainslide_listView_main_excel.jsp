...
String fileName = "사용중인 메인슬라이드 리스트.xls";
...
<table>
	<thead>
		<tr></tr>
		<tr class="background">
			<th colspan="${colspan}" class="title">사용중인 메인슬라이드 리스트</th>
		</tr>
		<tr></tr>
		<tr>
...
<table id="dt_basic">
	<thead>
		<tr class="background2">
			<th class="thsize">순번</th>
			<th class="thsize">홈페이지</th>
			<th class="thsize">위치</th>
	...
	<tbody>
		...
		<tr>
			<td class="tdName">${model.MM_LEVEL }</td>
			<td class="tdName">${model.MM_MN_MAINNAME }</td>
			<td class="tdName">${model.LOCATION_NAME }</td>
			<td class="tdName">${model.MM_TITLE }</td>
			<td class="tdName">${model.MM_URL }</td>
			<td class="tdName">${model.MM_REGNM }</td>
		</tr>
		</c:forEach>
	</tbody>
</table>
