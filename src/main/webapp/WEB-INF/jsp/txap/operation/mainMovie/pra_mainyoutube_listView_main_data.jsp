...
<table class="pagingTable table table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr>
			...
			<th class="arrow" data-index="6">유형</th>
	...
	<tbody>
		...
		<tr>
			<td class="text-center">
				<section>
					<label class="select">
						<select class="input-sm" onchange="fn_updateLevel('${model.BM_KEYNO }','${model.BM_LEVEL }',this.value, '${model.BM_MN_KEYNO }','${model.BM_TYPE }')">
							<c:forEach begin="1" end="${model.TOTAL_CNT}" var="level">
								<option value="${level }" ${level eq model.BM_LEVEL ? 'selected':'' }>${level }</option>
							</c:forEach>
						</select> <i></i>
				   </label>
				</section>
			...
			<td>
				<button type="button" class="btn btn-default btn-xs" onclick="pf_useBanner('${model.BM_KEYNO}','${model.BM_MN_KEYNO }','${model.BM_TYPE }','N','${model.BM_LEVEL}')"><i class="fa fa-rotate-left"></i> 중지</button>
			</td>
		</tr>
		</c:forEach>
	</tbody>
