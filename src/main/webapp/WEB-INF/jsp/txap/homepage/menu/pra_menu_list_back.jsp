...
<!-- 							<i class="fa fa-edit" style="color:red; "></i> : 페이지 상세보기  or 페이지 등록, -->
					...
					<div class="tree">
  						<ul>
    						<li class="parent_li" data-key="${menu.MN_HOMEDIV_C }"> 
								<span class="label label-primary">
									<i class="fa fa-lg fa-plus-circle">${HOMEDIV_C_NM }</i>
								</span>
								...
								<%-- <button type="button" class="btn btn-success btn-xs " onclick="pf_OpenAuthorityManager('${menu.MN_KEYNO}','${menu.MN_URL}','${menu.MN_PAGEDIV_C }')"><i class="fa fa-sign-in"></i></button> --%>
  								<div class="group">
  									<ul id="depth1Group"></ul>
								</div>
							</li>
						</ul>
					</div>
...
$(document).ready(function() {
	...
	//template1 홈페이지 추가 초기 셋팅
	pf_setHmTiles('template1');
	...
	$(document).on('click','.parent_li > span.label', function(e) {
		...
		if (children.is(':visible')) {
			...
			$(this).parent('li.parent_li')
					.find('ul')
						.hide()
						.html(pf_getSubList($(this).parents('li.parent_li:first').data('key')))
						.show('fast');
...
function pf_checkShowAndHide(value,action,childCnt){
	...
	//게시판 타입 show/hide
  	...
  	$('.RESEARCH').click(function(){
  		pageRatingShowHide(action)
  	});
  	
  	//공공누리 show/hide
  	$('.GONGGONGNULI').click(function(){
  		gonggnognuliShowHide(action)
  	});
...
function pf_create_layerDialog(a,b,c,d){
	...
	$('#' + a).dialog({
		autoOpen : false, width : 800,  resizable : true, modal : false, title : c,
		buttons : [{ 
...
function pf_Home_insert(){
	if( allInputCheck_notNull('#form_newHome') ){
  		...
  		if($('input[name=HM_TILES-radio]:checked').val() == 'custom'){
      	...
      	success: function(data) {
			...
			var menuCnt = data.MenuCnt;
      	 	if(result){
				...
				//최종 수정일/수정자 등록, 수정일/수정자 없을 경우 등록일/등록자
				var MODNM = result.MN_MODNM;
				var MODDT = result.MN_MODDT;
				if(MODNM == null || MODNM == ""){MODNM = result.MN_REGNM}
				if(MODDT == null || MODDT == ""){MODDT = result.MN_REGDT}
				$("#Mainmenu-update-lastREGNM").text(MODNM);
				$("#Mainmenu-update-lastREGDT").text(MODDT);
				...
				var gonggondCK= result.MN_GONGNULI_YN;
				if(typeof gonggondCK == 'undefined'){gonggondCK = 'N'}
				$("input[name=Mainmenu-update-GONGNULI_YN][value="+gonggondCK+"]").prop('checked',true);
				gonggnognuliShowHide('update')
				if(result.MN_GONGNULI_YN == 'Y'){
					$("input[name=Mainmenu-update-GONGNULI_TYPE][value="+result.MN_GONGNULI_TYPE+"]").prop('checked',true);
				}
				...
				if(result.MN_DU_KEYNO == 'DU_0000000000'){
					$('.Mainmenu-update-DirectSelect').prop('checked',true);
					$("#Mainmenu-update-MANAGER").val(result.MN_MANAGER);
					$("#Mainmenu-update-MANAGERDEP").val(result.MN_MANAGER_DEP);
					$("#Mainmenu-update-MANAGERTEL").val(result.MN_MANAGER_TEL);
				}else{
					$('.Mainmenu-update-DirectSelect').prop('checked',false);
					$('#Mainmenu-update-DEPARTMENT').val(result.DN_HOMEDIV_C);
					pf_chageDepartMentCategory('update',result.DN_HOMEDIV_C,'load',result.DU_KEYNO)
				}
					</c:otherwise>
				</c:choose>
...
function pf_Set_MenuInsertView(homekey,key,lev,url){
  	...
  	if(lev >= Number('${homeData.HM_MENU_DEPTH }') - 1 ){
   		...
   		initPageDiv = '${MENU_TYPE_BOARD}';
   		
  	}else{
    	boardOption.css("display","block");
    	initPageDiv = '${MENU_TYPE_SUBMENU}';
  	}
   	
  	$('#Mainmenu-insert-PAGEDIV_C').val(initPageDiv)
   	

	pf_checkShowAndHide(initPageDiv,'insert',0);
...
//홈페이치 타일즈 관련 셋팅
function pf_setHmTiles(value){
	var obj = $('#form_newHome input[name=HM_TILES]');
	$("#form_newHome select[name=HM_MENU_DEPTH]").attr("disabled", false);
 	if(value == 'custom'){
		...
		obj.val(value)
		if(value == 'template1'){
			$('#form_newHome select[name=HM_MENU_DEPTH]').val(3)
		}else if(value == 'template2'){
			$('#form_newHome select[name=HM_MENU_DEPTH]').val(2)
		} 
		$("#form_newHome select[name=HM_MENU_DEPTH]").attr("disabled", "disabled");
 	}
}
...
function pf_chageDepartMentCategory(type,value,type2,key){
	var temp = '<option value="">선택하세요.</option>'
	if(value){
		$.ajax({
			type: "POST",
			url: "/txap/operation/org/data2Ajax.do",
			async: false,
			data: {
				"DN_HOMEDIV_C"	: value
			},
 	...
 	$('#Mainmenu-'+type+'-DEPARTMENTUSER').html(temp).select2();
	if(type2 == 'load'){
		$('#Mainmenu-'+type+'-DEPARTMENTUSER').val(key).trigger('change');
	}
}


//페이지 평가 show/hide
function pageRatingShowHide(a){
	var value = $('input[name="Mainmenu-'+a+'-RESEARCH"]:checked').val();
	if(value == 'Y'){
		$('.pageRating').show();
	}else{
		$('.Mainmenu-'+a+'-DirectSelect').prop('checked',false);
		$('#Mainmenu-'+a+'-DEPARTMENT').val('');
		pf_chageDepartMentCategory(a,$('#Mainmenu-'+a+'-DEPARTMENT').val())
		$('.pageRating, .pageRatingInfo').hide()
	}
	pf_directSelectForm(a)
}

//페이지평가 직접 입력 선택시
function pf_directSelectForm(a){
	if($('.Mainmenu-'+a+'-DirectSelect').is(':checked')){
		$('#Mainmenu-'+a+'-DEPARTMENT').val('')
		pf_chageDepartMentCategory(a,$('#Mainmenu-'+a+'-DEPARTMENT').val())
		$('#Mainmenu-'+a+'-DEPARTMENT').attr('disabled', true);
		$('#Mainmenu-'+a+'-DEPARTMENTUSER').attr('disabled', true);
		$('.pageRatingInfo').show()
	}else{
		$('.pageRatingInfo').hide();
		$('#Mainmenu-'+a+'-DEPARTMENT').attr('disabled', false);
		$('#Mainmenu-'+a+'-DEPARTMENTUSER').attr('disabled', false);
		$('#Mainmenu-'+a+'-MANAGERDEP').val('');
		$('#Mainmenu-'+a+'-MANAGER').val('');
		$('#Mainmenu-'+a+'-MANAGERTEL').val('');
	}
}

//공공누리
function gonggnognuliShowHide(a){
	var value = $('input[name="Mainmenu-'+a+'-GONGNULI_YN"]:checked').val();
	if(value == 'Y'){
		$('.gonggnogType').show();
// 		$('input[name="Mainmenu-'+a+'-GONGNULI_TYPE"]').each(function(i){
// 			$(this).val(i)
// 		})
	}else{
		$('.gonggnogType').hide()
