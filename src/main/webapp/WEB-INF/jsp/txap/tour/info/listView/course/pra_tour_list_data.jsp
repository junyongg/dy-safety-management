...
<table id="dt_basic" class="pagingTable table table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr>
			...
			<th class="hasinput">
				<select class="form-control search-control" data-searchindex="2" style="width:100%;">
		            ...
		            <c:forEach items="${courseTypeList }" var="courseType">
						<option value="${courseType.SC_CODENM }">${courseType.SC_CODENM }</option>
					</c:forEach>
		         </select>
			</th>
			<th class="hasinput">
			...
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="4" placeholder="관람소요시간 검색" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="5" placeholder="추천수 검색" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="6" placeholder="사용여부 검색" />
			</th>
			<th class="hasinput">
		...
		<tr>
			...
			<th class="arrow" data-index="4">관람소요시간</th>
			<th class="arrow" data-index="5">추천수</th>
			<th class="arrow" data-index="6">사용여부</th>
			<th class="arrow" data-index="7">등록자</th>
			<th class="arrow" data-index="8">등록일</th>
		</tr>
	</thead>
	<tbody>
		<c:if test="${empty resultList }">
		...
		<tr>
			<td>${model.COUNT}</td>
			<td>${model.TCM_TYPE_NAME}</td>
			<td>
				<a href="javascript:;" onclick="pf_moveDetail('${model.TC_KEYNO}')">${model.TC_TITLE}</a>
			</td>
			<td>${model.TCM_TIME}</td>
