...
<section id="sub-content">
		<div class="inner1200"> <!--inner 시작 -->
			...
			<div id="sub-con-start">
                ...
                <div class="support_login_box">
                    ...
                    <input type="hidden" name="TH_BNKEYNO2" id="TH_BNKEYNO2" value="${hireDTO.TH_BNKEYNO2}">
                        <fieldset>
                            <legend>로그인</legend>
                        ...
                        <div class="log-btn-box">
                            <button type="button" onclick="UpdatePage()" class="login-btn1 purple">확인</button>
                        </div>
                    </form:form>
                </div>
	...
	function UpdatePage(){
		...
		$.ajax({
			type 	:	"post",
			url		:	"/hire/loginAjax2.do?${_csrf.parameterName}=${_csrf.token}",
	...
	function PasssYN(){
		...
		$.ajax({
			type 	:	"post",
			url		:	"/hire/PassAjax.do?${_csrf.parameterName}=${_csrf.token}",
			data	:	$('#form').serializeArray(),
			success :	function(data){
				if(data == 'Y'){
					alert("축하드립니다. 합격하였습니다.")
					$("#form").attr('action','/jact/hire/mypage.do');
					$("#form").submit();
				}else{
					alert("죄송합니다. 불합격 되었습니다. 다음에 다시 신청부탁드립니다.")
					return false;
				}
			},
			error	:	function(){
