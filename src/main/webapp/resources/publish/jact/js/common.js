...
$(function(){
   ...
   //햄버거 메뉴 - 사이트맵
	...
	$('.m-gnb > li').on('click',function(){
		$('.m-gnb .subMenu').stop().show();
		$('.subMenu-Wrap').stop().show();
	});
	
  $('.m-gnb > li > a').on('focus',function(){
		$('.m-gnb .subMenu').stop().show();
		$('.subMenu-Wrap').stop().show();
	});
  
	$('#hd').on('mouseleave',function(){
		$('.m-gnb .subMenu').stop().hide();
		$('.subMenu-Wrap').stop().hide();
	});

  	$('#hd > div.nav-wrap > div > div.nav-b > ul > li:nth-child(6) > ul > li:nth-child(2) > a').on('blur',function(){
		$('.m-gnb .subMenu').stop().hide();
		$('.subMenu-Wrap').stop().hide();
	});
  
	//메뉴뎁스3 버튼
	$('.subMenu > li .subMoreBtn .more').on('click',function(){
		if($(this).hasClass('active')){ //active가 있으면
			//active 지우기
			$(this).removeClass('active');
			$(this).find('i').removeClass('xi-minus-thin').addClass('xi-plus-thin'); //마이너스바꾸기
		} else { //active가 없으면
			//초기화
			...
			$(this).find('i').removeClass('xi-plus-thin').addClass('xi-minus-thin'); //마이너스바꾸기
		}
	});
  ...
  //서브탑 부분
   ...
   //부서 및 주요업무 소개 조직 아코디언
    ...
    $('.organi-accodian-wrap .title-b').on('click',function(){
    	...
    	if($(this).is('.active')) {
    		...
    		workArccordionUp()
    		$(this).addClass('active');
    		//$(this).next('.con-b').stop().slideDown();
    	}

    });
    // 사이드 퀵메뉴
    var $sidebar = $('.side-bar_01');
    if ($sidebar.length){
    ...
    $(window).on("scroll",function(){
    	...
    	if( windowTop >= quickPos) {
	    	$('.side-bar_01').css('top',windowTop - quickPos + 50)
	    } else {
			$('.side-bar_01').css('top',50)
	    }
    });
  
  
  //통화문화이용권 탭
  tabFucNomal('.in_s_tab_01 li','.in_s_tab_01_con');
...
//마이페이지 메뉴 펼치기
function myPageMenuShow(user) {
	if(user == null || user == '') {
		location.href="/jact/member/login.do"
	}else{
