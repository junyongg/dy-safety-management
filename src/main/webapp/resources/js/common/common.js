 ...
 * @param filekey
 */
function cf_download2(filepath,filename){
	var options = '?filepath='+filepath;
	if(filename){
		options += '&filename='+filename;
	}
	location.href="/async/file/download.do"+options;
}
...
function cf_smallBoxConfirm(title, content, func, bgColor, icon){
	...
	content = content + " <p class='text-align-right'><a href='javascript:;'" +func+" class='btn btn-primary btn-sm'>Yes</a> <a href='javascript:void(0);' class='btn btn-danger btn-sm'>No</a></p>";
	var options = {
		title : title,
		content : content,
		color : bgColor,
