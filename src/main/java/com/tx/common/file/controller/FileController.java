...
public class FileController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/async/file/PDFdownload.do")
	@ResponseBody
	public void MultiFilePDF(Model model
			, HttpServletRequest req 
			, HttpServletResponse res 
			, @ModelAttribute FileSub FileSub
			) throws Exception{
			...
			FileSub = Component.getData("File.AFS_FileSelectFileSub_PDF", FileSub);
//			//FileDownload
			FileDownloadTools.FileDownload(FileSub, req, res);
	}
	...
	public FileSub MultiFileUpLoad(HttpServletRequest req
		...
		, @RequestParam(value="BT_FILE_IMG_WIDTH",defaultValue="0") Integer BT_FILE_IMG_WIDTH
		, @RequestParam(value="BT_FILE_IMG_HEIGHT",defaultValue="0") Integer BT_FILE_IMG_HEIGHT
		) throws Exception{
		//등록자, 
		if(req.getSession().getAttribute("userInfo")!=null){
			...
			if( !UI_KEYNO.isEmpty() ){
				...
				if(BT_FILE_IMG_WIDTH == 0 && BT_FILE_IMG_HEIGHT == 0 ) {
					FileSub = FileUploadTools.FileUpload(req, UI_KEYNO, new FileSub(FS_FM_KEYNO, null));
				}else {
					 FileSub = FileUploadTools.FileUpload(req, UI_KEYNO, new FileSub(FS_FM_KEYNO, null).IS_MAKE_THUMBNAIL().THUMB_WIDTH(BT_FILE_IMG_WIDTH).THUMB_HEIGHT(BT_FILE_IMG_HEIGHT));
				}
	...
	public String MultiFileDelete(HttpServletRequest req
		...
		if(keyno != null){
			...
			keyList = new String[]{CommonService.getKeyno(keyno, "FS")};
